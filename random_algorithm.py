...
POPULATION_SIZE = 100
NUMBER_OF_GENERATIONS = 351
MAX_GENERATIONS_STAGNATED = 351
...
def selection_and_crossover(population):
    ...
    for i in range(min_descendents_count):
        descendents.append(mutate_individual(selected_pop[0]))
...
def selection(population_fitness_scores):
    ...
    num_elitist_selections = math.ceil(num_selections)
    ...
    return selected_elite_individuals



def mutate_individual(individual):
    new_individual_genes = deepcopy(individual.genes)
    # Mutating full random
    n_genes_to_mutate= random.randint(1, (len(individual.genes)))
